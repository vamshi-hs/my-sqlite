...
void deserialize_row(void* source,Row * destination) {
  ...
  memcpy(&(destination->username) ,USERNAME_OFFSET + source, USERNAME_SIZE);
  memcpy(&(destination->email) , EMAIL_OFFSET+source, EMAIL_SIZE);
}
...
ExecuteResult execute_select(Statement *statement,Table* table){
  ...
  for (uint32_t i = 0; i < table->num_rows; i++){
    ...
    print_row(&row);
    /* if (row.id == 1) { */
      /* printf("Your WiFi has been hacked\n"); */
      /* exit(1); */
    /* } */
  }
...
int main1(){
  printf("Testing.....\n");
  Statement test_insert_statement =  {
  ...
  Row* row_to_insert = &(test_insert_statement.row_to_insert);
  serialize_row(row_to_insert,row_slot(table,table->num_rows));
  table->num_rows++;
  Row row;
  for (uint32_t i = 0; i < table->num_rows; i++){
